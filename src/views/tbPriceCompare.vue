<template>
    <div class="priceContent">
        <el-button type="warning" class="searchHistory" @click="showDrawer"
            >查询历史</el-button
        >
        <h3 style="text-align:center;color:rgb(102,177,255);margin-top:4.3rem;">
            各大平台商品历史价格查询
        </h3>
        <!-- 查询表单 -->
        <div class="formBox">
        ...
        <div class="itemBox" v-if="recvData.spName">
            <div class="itemWrapper">
                ...
                <div class="title">
                    <span>
                        <span class="source">【{{ recvData.siteName }}】</span>
                        {{ recvData.spName }}
                    </span>
                    <span style="color:#666"
                        >销量：{{ recvData.tmp.commentCount }}</span
                    >
                    <div class="curprice">
                        <span>
                            <span class="keywords"
                                >￥{{ recvData.currentPrice }}</span
                            >
                        </span>
                        <el-button
                            size="small"
        ...
        <div class="couponInfo" v-if="couponInfo.category_id">
            <el-card>
                <div class="infoTitle">
                    <h3
                        style="text-align:center;font-size:16px;margin:0.5rem 0;"
                    >
                        商品/ 优惠券信息
                    </h3>
                </div>
        ...
        <div class="chartsBox" v-if="recvData.spName">
            <div class="trend">
                ...
                <span
                    class="keywords"
                    v-html="recvData.changPriceRemark"
                ></span>
                <br />
                <span>
                    最低价格：
                    <span class="keywords">￥{{ recvData.lowerPrice }}</span>
                    ({{ recvData.lowerDateFormat }})
                </span>
            </div>
...
export default {
    ...
    methods: {
            ...
            this.$refs.formRef.validate(async valid => {
                if (!valid) {
                    ...
                    let { data: res } = await this.$http.get(
                        `https://api.rambuild.cn/tools/priceHistory?url=${this.formData.goodsLink}`
                    );
                    if (res.data.status == 200) {
                        ...
                        // 格式化最低价格日期
                        let tmpDate = this.recvData.lowerDate.substr(6, 13);
                        this.recvData.lowerDateFormat = dayjs(
                            parseInt(tmpDate)
                        ).format("YYYY/MM/DD");
        ...
        solvePriceTrends() {
            let rawData = this.recvData.datePrice;
        ...
        async findCoupon() {
            ...
            if (queryUrl.includes("taobao") || queryUrl.includes("tmall")) {
                ...
                let { data: res } = await this.$http.get(
                    `https://api.rambuild.cn/tools/tbk/getCoupon?queryStr=${this.recvData.spUrl}`
                );
                this.getCouponLoading = false;
                // 如果该商品有优惠券
                if (res.data.tbk_dg_material_optional_response) {
                    if (
                        res.data.tbk_dg_material_optional_response.result_list
                            .map_data[0].coupon_share_url
                    ) {
                        this.$msg("success", "优惠券查询成功");
                        this.couponInfo =
                            res.data.tbk_dg_material_optional_response.result_list.map_data[0];
                        // 将二合一链接转换成淘口令
                        let { data: res2 } = await this.$http.get(
                            `https://api.rambuild.cn/tools/tbk/getTkl?url=https:${this.couponInfo.coupon_share_url}`
                        );
                        this.tkl = res2.data.tbk_tpwd_create_response.data.model
                            ? res2.data.tbk_tpwd_create_response.data.model
                            : "";
